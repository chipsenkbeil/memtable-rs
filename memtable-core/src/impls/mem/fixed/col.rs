...
pub struct MemFixedColumnTable<T: Default, const COL: usize> {
    #[cfg_attr(
        feature = "serde-1",
        serde(
            bound(
...
impl<T: Default, const COL: usize> MemFixedColumnTable<T, COL> {
...
impl<T: Default, const COL: usize> Table for MemFixedColumnTable<T, COL> {
    type Data = T;
...
impl<T: Default, const COL: usize> From<Vec<[T; COL]>> for MemFixedColumnTable<T, COL> {
...
impl<T: Default, const COL: usize> IntoIterator for MemFixedColumnTable<T, COL> {
    type Item = (Position, T);
    type IntoIter = ZipPosition<T, IntoCells<T, MemFixedColumnTable<T, COL>>>;
...
impl<T: Default, const COL: usize> Index<(usize, usize)> for MemFixedColumnTable<T, COL> {
    type Output = T;
...
impl<T: Default, const COL: usize> IndexMut<(usize, usize)> for MemFixedColumnTable<T, COL> {
...
mod tests {
    ...
    fn row_cnt_should_be_dynamic() {
        let table: MemFixedColumnTable<usize, 0> = MemFixedColumnTable::new();
        assert_eq!(table.row_cnt(), 0);

        let mut table: MemFixedColumnTable<usize, 4> = MemFixedColumnTable::new();
    ...
    fn col_cnt_should_match_fixed_row_size() {
        ...
        let table: MemFixedColumnTable<usize, 3> = MemFixedColumnTable::new();
    ...
    fn insert_cell_should_return_previous_cell_and_overwrite_content() {
        let mut table: MemFixedColumnTable<usize, 3> = MemFixedColumnTable::new();
    ...
    fn remove_cell_should_return_cell_that_is_removed() {
        let mut table = MemFixedColumnTable::from(vec![[1, 2], [3, 4]]);
    ...
    fn truncate_should_remove_cells_outside_of_row_capacity_count() {
        let mut table =
            MemFixedColumnTable::from(vec![["a", "b", "c"], ["d", "e", "f"], ["g", "h", "i"]]);
    ...
    fn shrink_to_fit_should_adjust_row_count_based_on_cell_positions() {
        let mut table: MemFixedColumnTable<&'static str, 3> = MemFixedColumnTable::new();
        assert_eq!(table.row_cnt(), 0);
        assert_eq!(table.col_cnt(), 3);
    ...
    fn index_by_row_and_column_should_return_cell_ref() {
        let table = MemFixedColumnTable::from(vec![[1, 2, 3]]);
    ...
    fn index_mut_by_row_and_column_should_return_mutable_cell() {
        let mut table = MemFixedColumnTable::from(vec![[1, 2, 3]]);
        table[(0, 1)] = 999;

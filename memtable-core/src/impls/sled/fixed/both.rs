...
#[repr(transparent)]
pub struct MemFixedTable<T: Default, const ROW: usize, const COL: usize>(
    #[cfg_attr(
        feature = "serde-1",
        serde(
            bound(
...
impl<T: Default, const ROW: usize, const COL: usize> MemFixedTable<T, ROW, COL> {
    ...
    pub fn iter(&self) -> ZipPosition<&T, Cells<T, MemFixedTable<T, ROW, COL>>> {
        self.into_iter()
    }
}

impl<T: Default, const ROW: usize, const COL: usize> Default for MemFixedTable<T, ROW, COL> {
    fn default() -> Self {
        Self(utils::default_table_array::<T, ROW, COL>())
    }
}

impl<T: Default, const ROW: usize, const COL: usize> Table for MemFixedTable<T, ROW, COL> {
    type Data = T;
...
impl<'a, T: Default, const ROW: usize, const COL: usize> IntoIterator
    for &'a MemFixedTable<T, ROW, COL>
{
    type Item = (Position, &'a T);
    type IntoIter = ZipPosition<&'a T, Cells<'a, T, MemFixedTable<T, ROW, COL>>>;
...
impl<T: Default, const ROW: usize, const COL: usize> IntoIterator for MemFixedTable<T, ROW, COL> {
    type Item = (Position, T);
    type IntoIter = ZipPosition<T, IntoCells<T, MemFixedTable<T, ROW, COL>>>;
...
mod tests {
    ...
    fn row_cnt_should_match_fixed_row_size() {
        let table: MemFixedTable<usize, 0, 0> = MemFixedTable::new();
        assert_eq!(table.row_cnt(), 0);

        let table: MemFixedTable<usize, 4, 0> = MemFixedTable::new();
    ...
    fn col_cnt_should_match_fixed_col_size() {
        ...
        let table: MemFixedTable<usize, 0, 4> = MemFixedTable::new();
        assert_eq!(table.col_cnt(), 4);
    }

    #[test]
    fn get_cell_should_return_ref_to_cell_at_location() {
        let table = MemFixedTable::from([["a", "b"], ["c", "d"]]);
    ...
    fn get_mut_cell_should_return_mut_ref_to_cell_at_location() {
        let mut table = MemFixedTable::from([["a", "b"], ["c", "d"]]);
    ...
    fn insert_cell_should_return_previous_cell_and_overwrite_content() {
        let mut table: MemFixedTable<usize, 3, 3> = MemFixedTable::new();
    ...
    fn remove_cell_should_return_cell_that_is_removed() {
        let mut table = MemFixedTable::from([[1, 2], [3, 4]]);
    ...
    fn index_by_row_and_column_should_return_cell_ref() {
        let table = MemFixedTable::from([[1, 2, 3]]);
    ...
    fn index_mut_by_row_and_column_should_return_mutable_cell() {
        let mut table = MemFixedTable::from([[1, 2, 3]]);
        table[(0, 1)] = 999;

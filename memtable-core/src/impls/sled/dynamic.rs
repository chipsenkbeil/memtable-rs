...
mod tests {
    ...
    fn new_should_calculate_row_and_column_counts_from_max_row_and_column() {
        let table = SledDynamicTable::from(make_empty_hashmap::<usize>());
        assert_eq!(table.row_cnt(), 0);
        assert_eq!(table.col_cnt(), 0);

        let table = SledDynamicTable::from(make_hashmap(vec![(3, 2, "some value")]));
        assert_eq!(table.row_cnt(), 4);
        assert_eq!(table.col_cnt(), 3);

        let table = SledDynamicTable::from(make_hashmap(vec![(3, 0, "value"), (0, 5, "value")]));
        assert_eq!(table.row_cnt(), 4);
    ...
    fn get_cell_should_return_ref_to_cell_at_location() {
        let table = SledDynamicTable::from(make_hashmap(vec![
            (0, 0, "a"),
            (0, 1, "b"),
            (1, 0, "c"),
            (1, 1, "d"),
    ...
    fn get_mut_cell_should_return_mut_ref_to_cell_at_location() {
        let mut table = SledDynamicTable::from(make_hashmap(vec![
            (0, 0, "a"),
            (0, 1, "b"),
            (1, 0, "c"),
            (1, 1, "d"),
    ...
    fn insert_cell_should_extend_max_row_size_if_adding_beyond_max_row() {
        let mut table = SledDynamicTable::new();
    ...
    fn remove_cell_should_return_cell_that_is_removed() {
        let mut table: SledDynamicTable<&'static str> =
    ...
    fn shrink_to_fit_should_adjust_row_and_column_counts_based_on_cell_positions() {
        let mut table: SledDynamicTable<&'static str> = SledDynamicTable::new();
        assert_eq!(table.row_cnt(), 0);
        assert_eq!(table.col_cnt(), 0);
